...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "InsertTvChannels")) {
  $insertSQL = sprintf("INSERT INTO rad24ublq (ChannelName, codes, daters, cates, keywds) VALUES (%s, %s, %s, %s, %s)",
...
$maxRows_Lradio = 20;
$Page_Lradio = 0;
if (isset($_GET['Page_Lradio'])) {
  $Page_Lradio = $_GET['Page_Lradio'];
}
$startRow_Lradio = $Page_Lradio * $maxRows_Lradio;

mysql_select_db($database_Ublq, $Ublq);
$query_Lradio = "SELECT * FROM rad24ublq";
$query_limit_Lradio = sprintf("%s LIMIT %d, %d", $query_Lradio, $startRow_Lradio, $maxRows_Lradio);
$Lradio = mysql_query($query_limit_Lradio, $Ublq) or die(mysql_error());
$row_Lradio = mysql_fetch_assoc($Lradio);

if (isset($_GET['Total_Lradio'])) {
  $Total_Lradio = $_GET['Total_Lradio'];
} else {
  $all_Lradio = mysql_query($query_Lradio);
  $Total_Lradio = mysql_num_rows($all_Lradio);
}
$totalPages_Lradio = ceil($Total_Lradio/$maxRows_Lradio)-1;
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "Page_Lradio") == false && 
        stristr($param, "Total_Lradio") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_Lradio = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_Lradio = sprintf("&Total_Lradio=%d%s", $Total_Lradio, $queryString_Lradio);
    ...
    <td valign="top"><table width="1003" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top" bgcolor="#FFFFFF"><table width="888" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="840" height="28" class="headering_black" style="padding-left:6px;">Ublq 2.0 Radio :</td>
              ...
              <table width="897" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td height="219" valign="top"><table width="895" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="416"><form action="<?php echo $editFormAction; ?>" id="InsertTvChannels" name="InsertTvChannels" method="POST">
                    ...
                    <input type="hidden" name="MM_insert" value="InsertTvChannels" />
                      ...
                      <td width="479"><div align="center"><img src="../imagine/Hollywood_Ticket.png" width="256" height="256" /></div></td>
                    </tr>
                  </table>                  </td>
              </tr>
                <tr>
                  <td valign="top"><img src="../imagine/Scissor Cut.gif" width="682" height="20" /><br /><?php if ($Total_Lradio > 0) { // Show if recordset not empty ?>
            <table width="894" border="0" cellpadding="2" cellspacing="0" bgcolor="#F9F9F9">
              <tr>
                <td width="63" height="28" bgcolor="#002137" class="headering"><div align="center" class="shopleftheading">ids : </div></td>
                <td width="552" bgcolor="#002137" class="headering"><div align="left">Channel Name</div></td>
              ...
              <?php do { ?>
              <tr>
                <td height="32" bgcolor="#F2F4F9" class="normal_textlinkRed"><div align="center"><?php echo $row_Lradio['ids']; ?></div></td>
                <td height="32" bgcolor="#FFFFFF" class="text3"><div align="left"><?php echo $row_Lradio['ChannelName']; ?></div></td>
                <td width="75" height="32" bgcolor="#FFFFFF" class="normal_textlink"><div align="center" class="styletextinglinkshop">
                    
                      <div align="center">
                        <?php if ($Total_Lradio > 0) { // Show if recordset not empty ?>
                        <a href="includes/del_rad.php?ids=<?php echo $row_Lradio['ids']; ?>" class="more">Del It </a>
                        <?php } // Show if recordset not empty ?>
                      </div>
                </div></td>
                <td width="100" height="32" bgcolor="#FFFFFF" class="text"><div align="left" class="text3">
                  <div align="center"><?php echo $row_Lradio['cates']; ?></div>
                </div></td>
                <td width="84" height="32" bgcolor="#FFFFFF" class="normal_textlink"><div align="center" class="styletextinglinkshop">
                  <div align="center"><a href="edit_rad.php?ids=<?php echo $row_Lradio['ids']; ?>" class="more">Edit</a></div>
                </div></td>
              </tr>
              <tr>
                <td height="1" colspan="6" bgcolor="#F2F4F9"></td>
              </tr>
              <?php } while ($row_Lradio = mysql_fetch_assoc($Lradio)); ?>
            </table>
            ...
            <table border="0" width="0">
              <tr>
                <?php if ($Page_Lradio > 0) { // Show if not first page ?>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lradio > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?Page_Lradio=%d%s", $currentPage, 0, $queryString_Lradio); ?>" class="footerlink">First</a>
                    <?php } // Show if not first page ?>
                    </div></td>
                <td width="70" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lradio > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?Page_Lradio=%d%s", $currentPage, max(0, $Page_Lradio - 1), $queryString_Lradio); ?>" class="footerlink">Previous</a>
                    <?php } // Show if not first page ?>
                    </div></td>
                <?php } // Show if not first page ?>
                <?php if ($Page_Lradio < $totalPages_Lradio) { // Show if not last page ?>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lradio < $totalPages_Lradio) { // Show if not last page ?>
                    <a href="<?php printf("%s?Page_Lradio=%d%s", $currentPage, min($totalPages_Lradio, $Page_Lradio + 1), $queryString_Lradio); ?>" class="footerlink">Next</a>
                    <?php } // Show if not last page ?>
                    </div></td>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lradio < $totalPages_Lradio) { // Show if not last page ?>
                    <a href="<?php printf("%s?Page_Lradio=%d%s", $currentPage, $totalPages_Lradio, $queryString_Lradio); ?>" class="footerlink">Last</a>
                    <?php } // Show if not last page ?>                
                    </div></td>
                <?php } // Show if not last page ?>
              </tr>
