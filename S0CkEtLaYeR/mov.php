...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "InsertTvChannels")) {
  $insertSQL = sprintf("INSERT INTO mov25ublq (ChannelName, codes, daters, cates, keywds) VALUES (%s, %s, %s, %s, %s)",
...
$maxRows_Lmovies = 20;
$Page_Lmovies = 0;
if (isset($_GET['Page_Lmovies'])) {
  $Page_Lmovies = $_GET['Page_Lmovies'];
}
$startRow_Lmovies = $Page_Lmovies * $maxRows_Lmovies;

mysql_select_db($database_Ublq, $Ublq);
$query_Lmovies = "SELECT * FROM mov25ublq";
$query_limit_Lmovies = sprintf("%s LIMIT %d, %d", $query_Lmovies, $startRow_Lmovies, $maxRows_Lmovies);
$Lmovies = mysql_query($query_limit_Lmovies, $Ublq) or die(mysql_error());
$row_Lmovies = mysql_fetch_assoc($Lmovies);

if (isset($_GET['Total_Lmovies'])) {
  $Total_Lmovies = $_GET['Total_Lmovies'];
} else {
  $all_Lmovies = mysql_query($query_Lmovies);
  $Total_Lmovies = mysql_num_rows($all_Lmovies);
}
$totalPages_Lmovies = ceil($Total_Lmovies/$maxRows_Lmovies)-1;
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "Page_Lmovies") == false && 
        stristr($param, "Total_Lmovies") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_Lmovies = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_Lmovies = sprintf("&Total_Lmovies=%d%s", $Total_Lmovies, $queryString_Lmovies);
    ...
    <td valign="top"><table width="1003" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td valign="top" bgcolor="#FFFFFF"><table width="888" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="840" height="28" class="headering_black" style="padding-left:6px;">Ublq 2.0 Movies :</td>
              ...
              <table width="897" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="901" height="219" valign="top"><table width="896" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="886"><div align="center"><table width="894" border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td height="219" valign="top"><table width="895" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="412"><form action="<?php echo $editFormAction; ?>" id="InsertTvChannels" name="InsertTvChannels" method="POST">
                    <table border="0" align="left" cellpadding="4" cellspacing="0">
                      <tr>
                        ...
                        <td width="303" bgcolor="#FFFFFF"><div align="left">
                          <input name="ChannelName" type="text" class="editbar" id="ChannelName" maxlength="30" />
                        </div></td>
                      </tr>
                      <tr>
                ...
                <tr>
                  <td valign="top"><img src="../imagine/Scissor Cut.gif" width="682" height="20" /><br /><?php if ($Total_Lmovies > 0) { // Show if recordset not empty ?>
            <table width="894" border="0" cellpadding="2" cellspacing="0" bgcolor="#F9F9F9">
              <tr>
                <td width="76" height="28" bgcolor="#002137" class="headering"><div align="center" class="shopleftheading">ids : </div></td>
                <td width="539" bgcolor="#002137" class="headering"><div align="left">Channel Name</div></td>
              ...
              <?php do { ?>
              <tr>
                <td height="32" bgcolor="#F2F4F9" class="normal_textlinkRed"><div align="center"><?php echo $row_Lmovies['ids']; ?></div></td>
                <td height="32" bgcolor="#FFFFFF" class="text"><div align="left" class="text3"><?php echo $row_Lmovies['ChannelName']; ?></div></td>
                <td width="75" height="32" bgcolor="#FFFFFF" class="normal_textlink"><div align="center" class="styletextinglinkshop">
                    
                      <div align="center">
                        <?php if ($Total_Lmovies > 0) { // Show if recordset not empty ?>
                        <a href="includes/del_mov.php?ids=<?php echo $row_Lmovies['ids']; ?>" class="more">Del It </a>
                        <?php } // Show if recordset not empty ?>
                      </div>
                </div></td>
                <td width="100" height="32" bgcolor="#FFFFFF" class="text"><div align="left" class="text3">
                  <div align="center"><?php echo $row_Lmovies['cates']; ?></div>
                </div></td>
                <td width="84" height="32" bgcolor="#FFFFFF" class="normal_textlink"><div align="center" class="styletextinglinkshop">
                  <div align="center"><a href="edit_mov.php?ids=<?php echo $row_Lmovies['ids']; ?>" class="more">Edit</a></div>
                </div></td>
              </tr>
              <tr>
                <td height="1" colspan="6" bgcolor="#F2F4F9"></td>
              </tr>
              <?php } while ($row_Lmovies = mysql_fetch_assoc($Lmovies)); ?>
            </table>
            ...
            <table border="0" width="0">
              <tr>
                <?php if ($Page_Lmovies > 0) { // Show if not first page ?>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lmovies > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?Page_Lmovies=%d%s", $currentPage, 0, $queryString_Lmovies); ?>" class="footerlink">First</a>
                    <?php } // Show if not first page ?>
                    </div></td>
                <td width="70" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lmovies > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?Page_Lmovies=%d%s", $currentPage, max(0, $Page_Lmovies - 1), $queryString_Lmovies); ?>" class="footerlink">Previous</a>
                    <?php } // Show if not first page ?>
                    </div></td>
                <?php } // Show if not first page ?>
                <?php if ($Page_Lmovies < $totalPages_Lmovies) { // Show if not last page ?>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lmovies < $totalPages_Lmovies) { // Show if not last page ?>
                    <a href="<?php printf("%s?Page_Lmovies=%d%s", $currentPage, min($totalPages_Lmovies, $Page_Lmovies + 1), $queryString_Lmovies); ?>" class="footerlink">Next</a>
                    <?php } // Show if not last page ?>
                    </div></td>
                <td width="50" align="center" class="reportchannel">
                  <div align="center">
                    <?php if ($Page_Lmovies < $totalPages_Lmovies) { // Show if not last page ?>
                    <a href="<?php printf("%s?Page_Lmovies=%d%s", $currentPage, $totalPages_Lmovies, $queryString_Lmovies); ?>" class="footerlink">Last</a>
                    <?php } // Show if not last page ?>                
                    </div></td>
                <?php } // Show if not last page ?>
              </tr>
